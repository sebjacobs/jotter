...
Project name today = `filepath.Base(git rev-parse --show-toplevel)` (`internal/git.go:13`). No config override, no remote fallback. Consequences: renaming a checkout directory silently orphans prior logs under the old name; two projects sharing a basename in different parents collide. (The branch-level `git branch -m` case is now handled — see Shipped.)

**Target shape — MVP set:**
- `jotter project mv <old> <new>` — `git mv logs/<old> logs/<new>` in the data repo + commit. The symmetric `jotter branch mv` / automatic branch-rename tracking has shipped (see Shipped); this item is now just the remaining project-level subcommands.
...
- **Branch-rename log continuity** (`feature/branch-rename`, release pending) — `git branch -m` no longer orphans a branch's logs. On the first on-branch write jotter stamps a stable id in the project repo's git config (`branch.<name>.jotter-id`, which survives the rename because git moves the whole `[branch]` section) plus a `<branch>.jsonl.id` sidecar; the next write after a rename git-mv's the logfile into place. Best-effort and scoped to on-branch writes, so scripted/cross-project writes are untouched. Ships with `jotter branch mv` (manual rename) and `jotter branch adopt` (proactively anchor existing branches so a rename before the next write is still followed). Spec + plan in `docs/specs/branch-rename/`. The cross-repo `adopt` sweep is deferred to a `proj`-side driver.
