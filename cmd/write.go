...
func init() {
	...
	writeCmd.Flags().String("type", "", "Entry type: start, checkpoint, note, break, finish, handover (required)")
...
func runWrite(cmd *cobra.Command, args []string) error {
	...
	if !internal.IsValidEntryType(entryType) {
		return fmt.Errorf("invalid entry type %q: must be one of start, checkpoint, note, break, finish, handover", entryType)
	}
